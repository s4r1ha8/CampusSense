      ...
      {loading ? (
        ...
        <>
          ...
          <div className="card" style={{marginBottom:20}}>
            <div className="card-title"><AlertTriangle size={16} color="var(--status-critical)"/> Active Alerts ({active.length})</div>
            {active.length === 0 ? (
              <div className="state-placeholder" style={{padding:'30px 0'}}>
                <CheckCircle2 size={32} color="var(--status-nominal)" style={{marginBottom:8}}/>
                ...
                <tbody>
                  {active.map(a => (
                    <tr key={a.alertId}>
                      ...
                      <td>{a.locationName}<br/><span style={{fontSize:11,color:'var(--on-surface-variant)'}}>{a.building}</span></td>
                      ...
                      <td style={{fontSize:12,color:'var(--on-surface-variant)'}}>{new Date(a.timestamp).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="card">
            ...
            {resolved.length === 0 ? (
              <p style={{color:'var(--on-surface-variant)',fontSize:14}}>No resolved alerts in the past 48 hours.</p>
            ) : (
              <table className="alert-table">
                <thead>
                ...
                <tbody>
                  {resolved.map(a => (
                    <tr key={a.alertId}>
                      ...
                      <td style={{fontSize:12,color:'var(--on-surface-variant)'}}>{a.resolvedAt ? new Date(a.resolvedAt).toLocaleString() : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
